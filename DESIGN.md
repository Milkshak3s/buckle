...
| macOS support | 14.2 (arm64) verified; newer releases need the §10 checks |
...
These were checked on macOS 14.2 (arm64, SIP on). **Re-verify all of them on newer macOS releases** (see §10).
...
- **exec bumps pidversion** (fixture `testdata/fixtures/macos14.2`). In an exec event, `process.audit_token` is the old image and `event.exec.target.audit_token` is the new one, so each exec'd image has its own token. `/bin/sh` execs `/bin/bash` on the same pid.
...
### Other
- `sandbox_check(pid, NULL, 0)` works without root on any pid. buckle doesn't use it (it would need cgo).
...
- **Claude Code in a live test** (`claude -p` headless with `sandbox.enabled`; interactive sessions are unverified):
  - The base64 payload decodes to the **tool_use ID** (`toolu_01…`), not the command, so buckle's `tag_command` column holds that ID. The real command is in the run's argv: `/bin/zsh -c "source <shell snapshot> && … eval '<command>' …"`.
...
- **Cursor** (`cursor-agent` CLI 2026.09.10; live buckle test and fixture `testdata/fixtures/macos14.2-cursor`):
...
- **Completeness:** the kernel drops many denial lines from non-platform binaries (§2), and there is no other unprivileged source. buckle accepts this: stored denials are a **lower bound**. Every process row records `is_platform_binary` from eslogger, so reports can show which counts may be incomplete. §4.4 candidates stay in scope even though their denials are the least reliable.
...
There's no orphan command. Inspect orphans with `sqlite3` directly.
...
Main entities: `watches`, `sessions`, `runs`, `processes`, `profiles` (by hash), `denials`, `orphans`, `run_env`.

**Schema v2:**
- **Change tracking:** every table gets a `rev` column. Triggers stamp it from a global counter on each insert and update. SQLite has a single writer, so revisions become visible in commit order and are safe to use as a sync cursor; any update gives a row a new, higher revision. Deletes (pruning) create no revisions.
- **`meta.db_instance`:** a random UUID identifying this DB.
- **Journal mode** is WAL, so readers don't block `watch`.
- **Privileges:** WAL files created by root `watch` are root-owned, so `ship` always needs sudo, and `query`/`report` need sudo while those files exist.
...
- **Upgrading:** `buckle migrate`, run without sudo, upgrades v1 or v2 databases in one transaction (backfilling revisions). New DBs start at the current version. `watch`, `ship`, `query` and `report` refuse older schemas.
...
- Go 1.27 or newer (arm64). modernc.org/sqlite needs at least 1.25.
- Xcode command line tools, to build `testdata/sbinit` when capturing fixtures.
- Full Disk Access for the terminal app that runs `sudo buckle watch`.

## 10. Re-verify on a new macOS release
...
- **Topology:** designed for a central server with a few Macs. The demo has one Mac reporting to a server on loopback, with no auth and no TLS. `serve` warns when `--addr` isn't loopback.
- **`sudo buckle ship`:** a foreground loop (every 10s by default). Each tick it asks the server for its cursor, reads up to 1000 rows with `rev > cursor` from one snapshot, posts them, and repeats immediately if the batch was full. An empty batch is still posted as a heartbeat. Host identity is `IOPlatformUUID` plus the hostname. On network or 5xx errors it retries with backoff (doubling from the interval, capped at 2 minutes); nothing is lost because the server owns the cursor. On a 4xx it exits. It never writes to the endpoint DB.
- **Wire protocol:** `GET /api/v1/cursor?host=<uuid>&db_instance=<uuid>` returns `{"rev": N}`. `POST /api/v1/ingest` takes `{host, db_instance, schema_version, rows: [{table, rev, data}]}` with at most 1000 rows and a 64 MiB body, and returns the new cursor. Errors: 400 for malformed batches (unknown table or column, missing key, non-scalar value, rev mismatch), 409 for a schema version mismatch, 413 for an oversized body.
- **`buckle serve`:** mirrors endpoint rows into `server.db`, keyed by host, DB instance and local id, with no foreign keys so rows may arrive before their parents (missing parents render as "unknown"). Rows upsert only when their rev is newer, so re-sends are no-ops. Reference columns keep their stored value when a later NULL arrives, since that NULL comes from endpoint pruning. Column names come from a fixed allowlist, never from the client. A recreated endpoint DB has a new `db_instance` and never overwrites older history. The server ignores endpoint pruning and keeps history forever. It runs with or without sudo; under sudo it keeps `server.db` owned by `SUDO_USER`.
- **UI:** server-rendered pages with no JS. A hosts page lists sessions, each with an agent badge (display names come from the compiled-in detector registry; unknown kinds show as stored), and "Untagged runs". The run page labels `tag_command` per agent. A run detail page shows the raw argv, the profile, and a timeline of process events and denials. Long arguments are shortened, with the full argv collapsed below. Times are shown in UTC and in the server's local time.
